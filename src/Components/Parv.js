...
export default function Parv({token}) {
    ...
    useEffect(async () => {
        const res = await axios.get(`https://hiji-p.herokuapp.com/${id}`,{
      ...
      const addComment=async()=>{
          try {
            const result = await axios.post(
                `https://hiji-p.herokuapp.com/comment/${id}`,
                {
                    comment:input
                },
                { headers: { authorization: "Bearer " + token } }
    ...
    const deletecomment =async (comment)=>{
        try {
            const result = await axios.put(`https://hiji-p.herokuapp.com/comment/${id}`,
        {comment:comment},
        {headers: { authorization: "Bearer " + token }})
